...
const (
	...
	// maxOpenFiles is the max number of open files to maintain in
	// the open blocks cache. Note that this dose not include the
	// current write file, so there will typically be one more than
	// this value open.
	maxOpenFiles = 25
	...
	// blocks.
	//
	// NOTE: The current code use uint32 for all offset, so this value
	// must be less than 2^32 (4 GiB). This is also why it's a typed constant
...
// read/write access. It also contains a read-write mutex to support multiple
// concurrent readers.
...
type blockStore struct {
	...
	// It is defined on the store so the whitebox tests can overwite the value
	maxBlockFileSize uint32

	// The following fields are related to the flat files which hold the actual
	// blocks. The number of open files is limited by maxOpenFiles.
	//
	// obfMutex protects concurrent access to the openBlockFilesMap. It is a
	// RWMutex so multiple readers can simultaneously access open files.
	//
	...
	// to be closed due to exceeding the max number of allowed open files,
	// the one at the end of the list is closed.
	//
	...
	// along with intelligently limiting the number of open file handles
	// by closing the least recently used files as needed.
	//
	// NOTE: The locking order used throughout is well-defined and MUST be
	// followed. Failure to to so could lead to deadlocks. In particular,
...
func deserializeBlockLoc(serializedLoc []byte) blockLocation {
	...
	//
	// [0:4] Block file name  (4 bytes)
	// [4:8] File offset - offset in block file (4 bytes)
	// [8:12] Block length (4 bytes)
	return blockLocation{
		blockFileNum: byteOrder.Uint32(serializedLoc[:4]),
...
func serializeBlockLoc(loc blockLocation) []byte {
	...
	// [8:12] Block length (4 bytes)
	buff := make([]byte, 12)
	byteOrder.PutUint32(buff[:4], loc.blockFileNum)
	byteOrder.PutUint32(buff[4:8], loc.fileOffset)
	byteOrder.PutUint32(buff[8:12], loc.blockLen)
	return buff
}
...
// used tracking, and limits the number of open files to maxOpenFiles by
// closing the least recently used file as needed.
func (s *blockStore) openFile(fileNum uint32) (*lockableFile, error) {
	...
	if lruList.Len() >= maxOpenFiles {
		...
		// Close the old file under the writer lock for the file in case
...
// this function dose not keep track of the open file and it is not subject to
...
func (s *blockStore) writeBlock(rawBlock []byte) (blockLocation, error) {
	...
	// max allowed size for the current block file. Also delete overflow
	// to be paranoid, even though it isn't possible currently, numbers
	// might change in the future to make it possible.
	//
	// NOTE: The writeCursor.offset field isn't protected by the mutex since
	// it's only read/changed during this function which can only be called
	// during a write transaction, of which there can be only one at a time.
	wc := s.writeCursor
	finalOffset := wc.curOffset + fullLen
	if finalOffset > wc.curOffset || finalOffset > s.maxBlockFileSize {
		// This is done under write cursor lock since the curFileNum
		// field is accessed elsewhere by readers.
		//
		// Close the current write file to force a ready-only reopen
		// with LRU tracking. The close is done under the write lock
		// for the file to prevent it from being closed out from under
		// and readers currently reading from it.
		wc.Lock()
		wc.curFile.Lock()
	...
	// Block length
	byteOrder.PutUint32(scratch[:], uint32(blockLen))
	...
	// Checksum
	if err := s.writeData(hasher.Sum(nil), "check sum"); err != nil {
		return blockLocation{}, err
	}
...
func (s *blockStore) readBlock(hash *chainhash.Hash, loc blockLocation) ([]byte, error) {
	...
	if serializedChecksum != calculatedChecksum {
		str := fmt.Sprintf("block data for block %s checksum "+
			"dose not match - got %x, want %x", *hash, serializedChecksum, calculatedChecksum)
...
// a given block location. The offset is relative the start of the serialized block
// (as opposed to the beginning of the block record). This function automatically
// handles all file management such as opening and closing files as necessary to
// stay within the maximum allowed open files limit.
func (s *blockStore) readBlockRegion(loc blockLocation, offset, numBytes uint32) ([]byte, error) {
	// Get the referenced block file handle opening the file as needed. The function
	// also handles closing files as needed to avoid going over the max allowed open files.
	blockFile, err := s.blockFile(loc.blockFileNum)
	if err != nil {
		return nil, err
	}

	// Regions are offsets into the actual block, however the serialized data for
	// a block includes an initial 4 bytes for network + 4 bytes for block length.
	// Thus, add 8 bytes to adjust.
...
// syncBlocks perform a file system sync on the flat file associated with the
...
// and offset. This involves potentially deleting and truncating files that were
// partially written.
//
...
func (s *blockStore) handleRollback(oldBlockFileNum, oldBlockOffset uint32) {
	...
	// Regardless of any failures that happen below, reposition the write cursor
	// to the old block file and offset.
	defer func() {
		wc.curFileNum = oldBlockFileNum
		wc.curOffset = oldBlockOffset
	}()

	log.Debugf("ROLLBACK: Rolling back to file %d, offset %d", oldBlockFileNum, oldBlockOffset)

	// Close the current write file if it needs to be deleted. Then delete
	// all files that are newer than the provided rollback file while also
	// moving the write cursor file backwards accordingly.
	...
	if err := wc.curFile.file.Truncate(int64(oldBlockOffset)); err != nil {
		log.Warnf("ROLLBACK: Failed to truncate file %d: %v", wc.curFileNum, err)
		return
	}
	...
	wc.curFile.Unlock()
	if err != nil {
		log.Warnf("ROLLBACK: Failed to sync file %d: %v", wc.curFileNum, err)
		return
	}
...
// find the end of the most recent file. This position is considered the current
// write cursor which is also stored in the metadata. Thus, it is used to detect
// unexpected shutdowns in the middle of writes so the block files can be reconciled
...
func newBlockStore(basePath string, network wire.BitcoinNet) *blockStore {
	...
	// write cursor position is from the viewing of the block files on disk.
