...
const (
	...
	// attemp to register with the name database type
	...
	// ErrKeyRequired indicates an attempt to insert a zero-length key.
	ErrKeyRequired
	...
	// ErrValueTooLarge indicates an attemp to insert a value that is larger
	// than max allowed value size. The max value size depends on the
	...
	// delete a bucket with an existing non-bucket key, attemping to create
...
// with caller rerqurests such as specifying invalid block regions or attempting
// to access data against closed database transactions, driver errors, errors
// retrieving data, and errors communicating with database server.
//
...
// be one of error codes provided by this package.
