...
const (
	...
	// high probability that the tree height is logarithmic, it is exceedingly
	// unlikely that the parent the parent stack will ever exceed this size
	// even for extremely large numbers of items.
	staticDepth = 128
	// nodeFieldSize is the size the fields of each node takes excluding
	...
	// unsafe. It consists of 24-bytes (ptr, len, cap of slice) for each key and value + 8 bytes for
	// each of the priority, lef, and right fields (24*2 + 8*3)
	nodeFieldSize = 72
)
...
func nodeSize(node *treapNode) uint64 {
	return nodeFieldSize + uint64(len(node.key)+len(node.value))
}
...
// hit during normal operation, however, since a treap's height is probabilistic,
// the overflow case needs to be handled properly. This approach is used
// because it is much more efficient for the majority case than dynamically
// allocating heap space every time the treap is iterated.
...
func (s *parentStack) Push(node *treapNode) {
	...
	// the item caused the compiler to make unneeded allocations. Also,
	// since the max number of items is related to the tree depth which
	// requires exponenttially more items to increase, only increase the cap
