...
func (t *Immutable) Put(key, value []byte) *Immutable {
	...
	// This ultimately allows key existence to be determined from the
	// value since an empty byte  slice is distinguishable from nil
	if value == nil {
		value = emptySlice
	}
	...
	for node := t.root; node != nil; {
		...
		newTotalSize := t.totalSize - uint64(len(node.value)) + uint64(len(value))
	...
	// Perform any rotations needed to maintian the min-heap and replace
...
// if it exists. The original immutable treap is returned if the key dose not exist.
func (t *Immutable) Delete(key []byte) *Immutable {
	...
	// There is nothing to do if the key dose not exist.
	if delNode == nil {
		return t
	}

	// When the only node in the tree is the root node and it is the one
	// being deleted, there is nothing to else to do besides removing it.
	parent := parents.At(1)
	...
	// Perform rotation to move the node to delete to a leaf position while
	...
	for delNode.left != nil || delNode.right != nil {
		...
		// Rotate left or right depending on which side the child onde
		// is one. This has the effect of moving the node to delete
		// towards the bottom of the tree while maintaining the min-heap
...
func (t *Immutable) ForEach(fn func(k, v []byte) bool) {
	// Add the root node and all children to the left of it to the list
	// of nodes to traverse and loop until they, and all of their child
