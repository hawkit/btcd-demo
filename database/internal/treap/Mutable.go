...
// dose not exist, both the node and the parent will be nil.
...
// relinkGrandparent rlinks the node into the treap after it has been rotated
// by changing the passed grandparent's left or right pointer, depending on
// where the old parent was, to point at the passed node. Otherwise, when
// there is no grandparent, it means the node is now the root of the tree, so
// update it accordingly.
func (t *Mutable) relinkGrandparent(node, parent, grandparent *treapNode) {
	// The node is now the root of the tree when there is no grandparents.
	if grandparent == nil {
		t.root = node
		return
	}
...
func (t *Mutable) Delete(key []byte) {
	// Find the node for the key along with its parent. There is nothing to do if the key
	// dose not exist.
