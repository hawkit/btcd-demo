...
func TestHashFuncs(t *testing.T) {
	...
	for _, tst := range tests {
		...
		result := HashB([]byte(tst.in))
		h := fmt.Sprintf("%x", result)
		if h != tst.out {
			t.Errorf("HashB(%q) = %s, want %s", tst.in, h, tst.out)
			return
		}
	}
	...
	for _, tst := range tests {
		...
		result := HashH([]byte(tst.in))
		h := fmt.Sprintf("%x", result)
		if h != tst.out {
			t.Errorf("HashH(%q) = %s, want %s", tst.in, h, tst.out)
			return
		}
	}
}
...
func TestDoubleHashFuncs(t *testing.T) {
	...
	for _, tst := range tests {
		...
		result := DoubleHashB([]byte(tst.in))
		h := fmt.Sprintf("%x", result)
		if h != tst.out {
			t.Errorf("DoubleHashB(%q) = %s, want %s", tst.in, h, tst.out)
			return
		}
	}
	...
	for _, tst := range tests {
		...
		result := DoubleHashH([]byte(tst.in))
		h := fmt.Sprintf("%x", result)
		if h != tst.out {
			t.Errorf("DoubleHashH(%q) = %s, want %s", tst.in, h, tst.out)
			return
		}
	}
}
